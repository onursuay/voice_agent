...
export const LEAD_COLUMNS: ColumnDef[] = [
  ...
  { key: 'phone', label: 'phone', type: 'phone', sortable: true, filterable: true, editable: true, width: 164 },
  { key: 'email', label: 'email', type: 'email', sortable: true, filterable: true, editable: true, width: 220 },
  ...
  { key: 'city_il', label: 'city_il', type: 'text', sortable: true, filterable: true, editable: false, width: 130 },
...
export function LeadGrid({ loading = false }: { loading?: boolean }) {
  ...
  const handleStageChange = useCallback((leadId: string, stageId: string) => {
    ...
    fetch(`/api/leads/${leadId}`, {
      ...
      .then((data) => {
        if (data && 'meta_sync' in data) {
          const notice = syncNoticeFromMeta(data.meta_sync as LeadMetaSync, t as (k: string, v?: Record<string, string | number>) => string);
          if (notice) setSyncNotice(notice);
        }
      })
      .catch(console.error);
  }, [stages, updateLead, flashSaved, setSyncNotice, t]);
  ...
  }, [stages, tRouting, members, locale, sourceLabel, stageLabel]);
