...
interface RoutingRulesSectionProps {
  ...
  stages: { id: string; name: string; slug?: string; color?: string }[];
  /** Üstteki "Yeni Otomasyon" butonu (her artışta) bu tek builder'ı açar. */
  openSignal?: number;
}

function RoutingRulesSection({ allRules, onRulesChange, members, stages, openSignal }: RoutingRulesSectionProps) {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      priority: editingId ? form.priority : routeRules.length,
  ...
  const getConditionSummary = (rule: AutomationRule): string | null => {
    ...
    const sep = (rule.trigger_config?.match as 'all' | 'any') === 'any' ? ' / ' : ' + ';
