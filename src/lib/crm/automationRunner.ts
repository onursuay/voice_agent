...
import { getNotificationSettings } from '@/lib/crm/notificationSettings';
...
export async function reconcileMetaAudienceSync(): Promise<{ checked: number; reconciled: number; failed: number }> {
  ...
  const { data: leads } = await supabase
    .from('leads')
    .select('id, email, phone, full_name, meta_ad_id, meta_capi_sent, organization_id, stage:crm_stages(*)')
    .is('deleted_at', null)
